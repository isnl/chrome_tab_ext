...
const emit = defineEmits<{
  ...
  "update:importance": [value: TodoImportance];
  insidePointer: [];
  close: [];
}>();
...
const datePickerLabel = computed(() => formatDateLabel(props.dueDate));
const timePickerLabel = computed(() => props.dueTime || "不提醒时间");
const hasDetails = computed(() => Boolean(props.dueDate || props.dueTime || props.importance !== "low"));
const detailsSummary = computed(() => {
  const parts = [
    props.dueDate ? datePickerLabel.value : null,
    props.dueTime ? props.dueTime : null
  ].filter(Boolean);
...
const calendarDays = computed(() => {
  ...
  return Array.from({ length: 42 }, (_, index) => {
    ...
    return {
      ...
      isSelected: props.dueDate === value,
...
function currentTimeParts() {
  return parseTimeInputValue(props.dueTime) ?? { hour: "09", minute: "00" };
}
...
function syncCalendarToDueDate() {
  datePickerMonth.value = startOfMonth(parseDateInputValue(props.dueDate) ?? new Date());
}

function setQuickDue(offset: number) {
  const date = new Date();
  date.setDate(date.getDate() + offset);
  emit("update:dueDate", toDateInputValue(date));
...
function isQuickDueActive(offset: number) {
  const date = new Date();
  date.setDate(date.getDate() + offset);
  return props.dueDate === toDateInputValue(date);
}
...
function isTimeHourSelected(hour: string) {
  return parseTimeInputValue(props.dueTime)?.hour === hour;
}

function isTimeMinuteSelected(minute: string) {
  return parseTimeInputValue(props.dueTime)?.minute === minute;
}
...
function containsTarget(target: EventTarget | null) {
  ...
  return Boolean(popoverRef.value?.contains(node) || props.anchorEl?.contains(node));
}
...
watch(
  ...
  async (isOpen) => {
    ...
    timePanelOpen.value = Boolean(props.dueTime);
    await nextTick();
    updatePosition();
  }
);
  ...
  <Teleport to="body">
    <Transition name="todo-details">
      <div
        ...
        :class="{ 'todo-details-popover--compact': compact }"
        :style="popoverStyle"
        @pointerdown.capture="emit('insidePointer')"
        @click.stop
      ...
      >
        ...
        <div class="todo-priority-swatches" aria-label="重要程度">
          <button
            ...
            @click="emit('update:importance', option.value)"
          >
...
.todo-details-popover--compact {
  padding: 9px;
}

.todo-details-popover--compact .todo-picker-grid {
  gap: 5px;
}

.todo-details-popover--compact .todo-picker-card {
  grid-template-columns: 18px minmax(0, 1fr);
  gap: 5px;
  padding: 6px;
}

.todo-details-popover--compact .todo-picker-card__icon {
  width: 18px;
  height: 18px;
  border-radius: 7px;
}

.todo-details-popover--compact .todo-priority-swatches {
  gap: 5px;
}

.todo-details-popover--compact .todo-calendar {
  padding: 7px;
}

.todo-details-popover--compact .todo-calendar__grid {
  gap: 3px;
}

.todo-details-popover--compact .todo-calendar__day {
  border-radius: 8px;
  font-size: 9px;
}

.todo-details-popover--compact .todo-time-picker {
  padding: 7px;
}

.todo-details-popover--compact .todo-time-columns {
  grid-template-columns: minmax(0, 1fr);
  gap: 7px;
}

.todo-details-popover--compact .todo-time-column__grid--minutes {
  grid-template-columns: repeat(6, minmax(0, 1fr));
}

.todo-details-popover--compact .todo-time-quick__item span {
  display: none;
}

.todo-details-popover--compact .todo-priority-swatch {
  padding: 6px 3px 5px;
}

.todo-details-popover--compact .todo-priority-swatch__dot {
  width: 10px;
  height: 10px;
}

.todo-details-popover--compact .todo-priority-swatch__hint {
  display: none;
}
</style>
